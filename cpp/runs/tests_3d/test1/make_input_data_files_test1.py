...
def generate_constant_temperature_profile_3d( amp , nx , ny , nz ):
...
def main():
    ...
    xx,yy,zz,initial_T  = generate_constant_temperature_profile_3d( amp_temp , settings.nx , settings.ny , settings.nz )
    initial_T           = initial_T.ravel()
    initial_T_file      = 'initial_temperature.ascii'
    np.savetxt( initial_T_file , initial_T , fmt='%1.8f' )
    os.system( builddir + 'preprocess_3d petscrc.dat ' + initial_T_file )
    ...
    os.system( builddir + 'preprocess_3d petscrc.dat ' + T_source_file )
    
    # Write the initial solution field to disk for petsc
    initial_U      = 0.005 * ( 2.0 * np.random.uniform(0,1,settings.nx*settings.ny*settings.nz) - 1.0 )
    initial_U_file = 'initial_soln.ascii'
    np.savetxt( initial_U_file , initial_U , fmt='%1.8f' )
    os.system( builddir + 'preprocess_3d petscrc.dat ' + initial_U_file )
    
    # Write temperature dirichlet field to disk for petsc
    x0       = settings.nx // 2
    y0       = settings.ny // 2
    amp_temp = 1.0
    xx,yy,zz,initial_T  = generate_gaussian_temperature_profile_on_3d_face( amp_temp , sigma_temp , settings.nx , settings.ny , settings.nz , x0 , y0 )
    initial_T           = initial_T.ravel()
    ...
    # Write temperature dirichlet field to disk for petsc (specifies thermal boundary conditions)
    x0       = settings.nx // 2
    y0       = settings.ny // 2
    amp_ch   = 1.0
    xx,yy,zz,initial_T  = generate_constant_temperature_profile_3d( amp_ch , settings.nx , settings.ny , settings.nz )
    initial_T           = initial_T.ravel()

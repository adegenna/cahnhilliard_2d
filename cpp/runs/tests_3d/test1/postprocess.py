...
def main( petscfile ):
    ...
    num_bin_files = int( ( subprocess.run( 'ls -1q c_*.bin | wc -l' , shell=True , stdout=subprocess.PIPE ) ).stdout )
    
    for i in range( num_bin_files ):
        timestamp  = '_{:0.4f}.bin'.format( i * settings.dt_out )
        solnfile_i = 'c' + timestamp
        print( builddir + 'postprocess ' + petscfile + ' ' + solnfile_i )
        os.system( builddir + 'postprocess ' + petscfile + ' ' + solnfile_i )

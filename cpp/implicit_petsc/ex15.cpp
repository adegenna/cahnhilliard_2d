...
int main(int argc,char **argv)
{
  ...
  if (Jtype == 0) { /* use user provided Jacobian evaluation routine */
    ierr = PetscPrintf(PETSC_COMM_WORLD,"JACOBIAN: user-provided\n");CHKERRQ(ierr);
    if (user.nstencilpts != 5)
      SETERRQ1(PETSC_COMM_WORLD,PETSC_ERR_SUP,"user Jacobian routine FormIJacobian() does not support nstencilpts=%D",user.nstencilpts);
    ...
    if (Jtype == 1) { /* slow finite difference J; */
      ierr = PetscPrintf(PETSC_COMM_WORLD,"JACOBIAN: matrix-free with non-optimized finite differences\n");CHKERRQ(ierr);
      ...
      ierr = PetscPrintf(PETSC_COMM_WORLD,"JACOBIAN: matrix-free with coloring for finite-differences\n");CHKERRQ(ierr);
  ...
  // ierr = PetscViewerBinaryOpen(PETSC_COMM_WORLD,"phi1",FILE_MODE_WRITE,&viewer_out);CHKERRQ(ierr);
  // ierr = VecView( u , viewer_out ); CHKERRQ(ierr);
